...
use crate::calculation::trip_cost::{calculate_flat_trip_cost, calculate_itemized_trip_cost};
use crate::domain::dto::{BudgetSummaryDto, TripCostPreviewDto, TripInputDto};
...
#[tauri::command]
pub fn delete_trip(state: State<'_, AppState>, id: Uuid) -> Result<BudgetSummaryDto, AppError> {
    let mut lock = state.project.lock().unwrap();
    let project = lock.as_mut().ok_or(AppError::NoProject)?;
...
pub fn preview_trip_cost(
    ...
    match &input.trip_type {
        TripType::Itemized {
            ...
            Ok(TripCostPreviewDto {
                ...
                accommodation_cost_per_instance: Some(
                    result.accommodation_cost_per_instance.to_string(),
                ),
                subsistence_cost_per_instance: Some(
                    result.subsistence_cost_per_instance.to_string(),
                ),
                domestic_transport_per_instance: Some(
                    result.domestic_transport_per_instance.to_string(),
                ),
        ...
        TripType::FlatAmount {
            ...
            let result =
                calculate_flat_trip_cost(*flat_amount_per_instance_eur, input.number_of_instances)?;
            Ok(TripCostPreviewDto {
