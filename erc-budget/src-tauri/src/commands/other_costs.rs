...
use crate::domain::dto::{BudgetSummaryDto, OtherCostInputDto};
    ...
    let item = project
        ...
        .ok_or_else(|| {
            AppError::NotFound(format!(
                "Other cost item {id} not found (or is a CFS item)."
            ))
        })?;
...
pub fn delete_other_cost(
    ...
    project
        .other_cost_items
        .retain(|i| i.id != id || i.is_cfs_item);
    if project.other_cost_items.len() == before {
        return Err(AppError::NotFound(format!(
            "Other cost item {id} not found or is a CFS item (use remove_cfs_item)."
        )));
    }
...
pub fn add_cfs_item(
    ...
    if amount_eur <= Decimal::ZERO {
        return Err(AppError::Validation(vec![crate::error::FieldError::new(
            "amount_eur",
            "INVALID_C3_AMOUNT",
            "CFS amount must be greater than zero.",
        )]));
    }
    ...
    if work_package_ids
        .iter()
        .any(|&wp| wp < 1 || wp > work_package_count)
    {
        return Err(AppError::Validation(vec![crate::error::FieldError::new(
            "work_package_ids",
            "WP_OUT_OF_RANGE",
...
#[tauri::command]
pub fn remove_cfs_item(state: State<'_, AppState>) -> Result<BudgetSummaryDto, AppError> {
    let mut lock = state.project.lock().unwrap();
    let project = lock.as_mut().ok_or(AppError::NoProject)?;
...
#[tauri::command]
pub fn dismiss_cfs_warning(state: State<'_, AppState>) -> Result<BudgetSummaryDto, AppError> {
    let mut lock = state.project.lock().unwrap();
    let project = lock.as_mut().ok_or(AppError::NoProject)?;
...
pub fn set_subcontracting(
    ...
    if amount_eur < Decimal::ZERO {
        return Err(AppError::Validation(vec![crate::error::FieldError::new(
            "amount_eur",
            "INVALID_SUBCONTRACTING",
            "Subcontracting amount cannot be negative.",
        )]));
    }
