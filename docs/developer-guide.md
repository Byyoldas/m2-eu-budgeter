...
- The Personnel sheet's "Work Package Timelines" table also computes each role's **Person-Months (PM)** per WP as an FTE-weighted formula (`months × FTE`), plus a live Total PM / Employment Months / Reconciled? check block, so the allocation is auditable directly in the spreadsheet. These PM cells (and the Total PM / Employment Months check rows) are displayed with `numFmt = '0.0'` — 1 decimal place, standard round-half-up — matching the EU Funding & Tenders Portal's own Person-Months display convention (a second decimal of 5 or higher rounds the first decimal up, e.g. 9.65 → 9.7). This is display formatting only: the underlying formula values stay full precision, so the reconciliation check compares exact numbers and can't drift from per-WP rounding. Column E doubles as both the first role's PM column here and the Roles table's FTE-fraction column further down the same sheet (headered "PM" too, matching the Personnel screen's own "PM (0–1)" field label, even though it's the raw 0–1 fraction rather than a Person-Months count) — their number formats must be set per-cell (`row.getCell(5).numFmt = ...`), never via `column.numFmt`, or one silently overwrites the other.
