...
- The Personnel sheet's per-Work-Package cost cells, and the row's Total column, are **live formulas**, not pulled values — every row talks to the others, and editing a role's salary/FTE/dates/increase in the sheet and letting Excel recalculate reproduces the same numbers the app computed. This is driven by a second hidden helper sheet, `_WPYearHelper` (see below), that reproduces the backend's Person-Months rounding rule inside the spreadsheet rather than requiring the workbook to trust a pulled number.
- **Person-Months (PM) drive cost, not just display.** `erc-core::calculation::personnel_cost::round_person_months` rounds `active_months × fte_fraction` to 1 decimal place, standard round-half-up (a second decimal of 5 or higher rounds the first decimal up, e.g. 2.37 → 2.4), matching the EU Funding & Tenders Portal's own Person-Months convention — and `annual_cost_eur` (CALC-03) is computed from that *rounded* PM, not the exact fractional value. `allocate_personnel_cost_by_wp` (CALC-20a) rounds each year's total WP-covered PM the same way to get that year's cost, then splits it across WPs in proportion to each WP's *unrounded* PM share.
- **`_WPYearHelper`** (hidden) has one row per `(role, project year)`, in `role_detail` order — a role's block of `duration_years` rows is always contiguous, so its Total is a single `SUM` over a row range. Each row computes, purely as formulas referencing that role's own input cells on the Personnel sheet: Active Months this year (CALC-03's overlap of `[start,end]` with the year's month range), PM rounded (`ROUND(activeMonths×FTE, 1)` — Excel's `ROUND` is round-half-away-from-zero, the same rule `round_person_months` uses), Salary this year (`baseMonthly×(1+increase/100)^year`), Annual Cost — CALC-03 (`PM rounded × salary`, which the Total column sums across years), Covered PM unrounded (the same idea restricted to months some WP actually covers, via `_WPMonthHelper`'s reciprocal row — this can be less than Active Months when some months fall outside every WP), Covered PM rounded, and Annual Cost from WPs (the year's priced, rounded WP-split basis). A role's per-WP cost cell on the Personnel sheet then sums, year by year, `IF(coveredTotal=0, 0, annualCostFromWPs × thisWPsCoveredPM / coveredTotal)` — a proportional split of each year's already-rounded pool. Unlike the Rust backend (which gives the last WP in a year the exact Decimal remainder to avoid a repeating-decimal residual, e.g. from 4/12), the Excel formulas split every WP proportionally; Excel's IEEE-754 doubles don't accumulate a Decimal-truncation artifact at this scale, so the difference between the two approaches is far below a cent and invisible at 2-decimal display.
- The Personnel sheet's "Work Package Timelines" table separately computes each role's Person-Months per WP as an FTE-weighted `SUMPRODUCT` formula (`months × FTE`) over `_WPMonthHelper`, plus a live Total PM / Employment Months / Reconciled? check block — this is a **purely informational audit table**, unrelated to the cost calculation above (it's unrounded, and it never feeds into money, in Excel or in Rust). It's a live, full-precision formula, displayed with `numFmt = '0.0'` for readability only. Column E doubles as both the first role's PM column here and the Roles table's FTE-fraction column further down the same sheet (headered "PM" too, matching the Personnel screen's own "PM (0–1)" field label, even though it's the raw 0–1 fraction rather than a Person-Months count) — their number formats must be set per-cell (`row.getCell(5).numFmt = ...`), never via `column.numFmt`, or one silently overwrites the other.
...
Because of the formulas, **`excelExporter.test.ts` doesn't just assert formula text** — it uses `hyperformula` (a spreadsheet-formula evaluation library, dev dependency) to actually evaluate the generated workbook and assert on the resulting numbers, cross-checked against hand-derived expected values. This is the pattern to follow for any future change to the formula-generating code: assert on evaluated output, not just that a formula string was produced. When building the `HyperFormula.buildFromSheets` grid for a Personnel-sheet test, remember to include **every** hidden helper sheet a formula might touch (`_WPMonthHelper`, `_WPYearHelper`) and `Budget Summary` (the Base Monthly formula references its TRY→EUR rate cell) — leaving one out doesn't fail the build, it silently turns every dependent cell into a `#REF!` `DetailedCellError` that only surfaces when you read the cell's value.
