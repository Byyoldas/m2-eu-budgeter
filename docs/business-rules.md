...
> ## ⚠ Current Implementation Notes (as of v1.9.0, 2026-09-21)
>
...
> - **PE-03 — Person-Months (PM) are displayed to 1 decimal place, standard round-half-up**, matching the EU Funding & Tenders Portal's own Person-Months display (a second decimal of 5 or higher rounds the first decimal up, e.g. 9.65 → 9.7; below 5 rounds down, e.g. 9.64 → 9.6). This applies to the Excel export's Personnel sheet — the per-WP PM column, the Total PM check row, and the Employment Months row it reconciles against. It's a display convention only: the underlying values stay full precision, so the reconciliation check can't be thrown off by per-WP rounding drift.
