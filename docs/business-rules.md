...
> ## ⚠ Current Implementation Notes (as of v1.11.0, 2026-09-21)
>
...
> - **PE-03 — Person-Months (PM) are rounded to 1 decimal place, standard round-half-up, and cost is computed from the rounded value.** `active_months × fte_fraction` (e.g. 3 months at FTE 0.79 = 2.37) rounds to 2.4 (a second decimal of 5 or higher rounds the first decimal up; below 5 rounds down), and `Annual Cost = Monthly Salary × rounded PM` — not the exact 2.37 — matching how the EU Funding & Tenders Portal itself computes requested personnel cost from its own rounded Person-Months figure. The per-WP split (personnel cost allocated across Work Packages) rounds the same way, once per year, before dividing proportionally across WPs, so the split always sums exactly to the same rounded-PM total — it never re-derives its own independently-rounded number. The Excel export's WP Timelines "Person-Months" audit table (a separate, purely informational block — see `docs/developer-guide.md` §13) also displays to 1 decimal for the same reason, but that one is cosmetic only; this rule is about the actual requested amount.
