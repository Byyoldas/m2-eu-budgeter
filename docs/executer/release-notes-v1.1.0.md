# ERC Execution v1.1.0 — Release Notes

*This is the first published release of ERC Execution — tagged and
released as `erc-execution/v1.1.0`, distinct from ERC Budget's own release
line. The version starts at 1.1.0 rather than 1.0.0 because the fixes
found during the first local test pass (see "Fixed since the original
1.0.0 build" below) landed before this was ever tagged or published.*
...
- **Personnel & Person-Month Tracking** — roster management and monthly
  ...
  **Time Declaration export** (new in 1.1.0 — see below).
...
- **Reports & Export** — Financial Report, Technical Report Annex, Risk
  ...
  Project Status Report as a printable PDF; and, new in this release, an
  **EU Grants Time Declaration export**.

### New in 1.1.0: EU Grants Time Declaration export

The Personnel screen now has a **Time Declaration** button per person. It
...
- **Some date-range validations are skipped when a project has no call
  opening date set** in the Budget App, including the new Time
  Declaration export's calendar-year mapping.
