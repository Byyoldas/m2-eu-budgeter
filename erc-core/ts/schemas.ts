/**
 * Shared Zod schemas for front-end form validation — used by both the
 * Budget Application and (once it exists) the Execution Application.
 *
 * These run instantly (no IPC round-trip) for basic field-level checks.
 * Cross-entity constraints are enforced server-side (see erc-core::validation).
 *
 * Hand-maintained (not generated) — unlike erc-core/bindings/*.ts, these have
 * no Rust equivalent to derive from, since Zod schemas encode UI-side
 * validation rules (min/max, required-field messages) that go beyond what a
 * DTO's shape alone expresses.
 */

import { z } from 'zod';

// Positive decimal string helper
export const decimalStr = (label: string) =>
  z.string().refine((v) => !isNaN(parseFloat(v)) && parseFloat(v) > 0, {
    message: `${label} must be a positive number.`,
  });

export const nonNegDecimalStr = (label: string) =>
  z.string().refine((v) => !isNaN(parseFloat(v)) && parseFloat(v) >= 0, {
