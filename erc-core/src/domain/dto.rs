//! Shared Data Transfer Objects (DTOs) for the IPC boundary.
//!
//! DTOs are what a frontend sends in and receives back. They are strictly
//! separate from the domain entities:
//! - Input DTOs: what the frontend sends to create/update entities.
//! - Output DTOs: what the backend returns after every mutation — including
//!   `BudgetSummaryDto`, the primary output of the calculation engine
//!   (`erc_core::calculation::budget_summary::calculate_budget_summary`).
//!
...
mod tests {
    ...
    fn personnel_role_input_deserializes_frontend_json() {
        for role_type in [
            ...
            assert!(
                result.is_ok(),
                "role_type '{role_type}' failed to deserialize: {:?}",
                result.err()
            );
        }
    }
    ...
    fn other_cost_input_deserializes_frontend_json() {
        ...
        assert!(
            result.is_ok(),
            "OtherCostInputDto failed to deserialize: {:?}",
            result.err()
        );
    }
    ...
    fn itemized_trip_input_deserializes_frontend_json() {
        ...
        assert!(
            result.is_ok(),
            "Itemized TripInputDto failed to deserialize: {:?}",
            result.err()
        );
        assert!(matches!(
            result.unwrap().trip_type,
            crate::domain::entities::TripType::Itemized { .. }
        ));
    }
    ...
    fn flat_amount_trip_input_deserializes_frontend_json() {
        ...
        assert!(
            result.is_ok(),
            "FlatAmount TripInputDto failed to deserialize: {:?}",
            result.err()
        );
        assert!(matches!(
            result.unwrap().trip_type,
            crate::domain::entities::TripType::FlatAmount { .. }
        ));
    }
}
