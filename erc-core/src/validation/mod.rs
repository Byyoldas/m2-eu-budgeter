//! Validation Engine — cross-entity business rule enforcement shared by the
//! ERC Budget and ERC Execution applications.
//!
...
pub fn validate_personnel_role(
    ...
    if dto.start_month < 1 || dto.start_month > max_month {
        errors.push(FieldError::new(
            ...
            "MONTH_OUT_OF_RANGE",
            format!(
                "Start month {} is outside the project duration of {max_month} months.",
                dto.start_month
            ),
        ));
    }
    if dto.end_month < 1 || dto.end_month > max_month {
        errors.push(FieldError::new(
            ...
            "MONTH_OUT_OF_RANGE",
            format!(
                "End month {} is outside the project duration of {max_month} months.",
                dto.end_month
            ),
        ));
    }
...
pub fn validate_trip(dto: &TripInputDto, work_package_count: u8) -> Result<(), AppError> {
    ...
    if dto.name.trim().is_empty() {
        errors.push(FieldError::new(
            "name",
            "REQUIRED",
            "Trip name or purpose is required.",
        ));
    }
...
mod tests {
    use super::*;
    use crate::domain::dto::{
        EquipmentItemInputDto, OtherCostInputDto, PersonnelRoleInputDto, ProjectConfigDto,
        TripInputDto,
    };
    ...
    fn test_val_eq_empty_name_returns_error() {
        ...
        dto.name = "".to_string();
        assert!(has_field_error(
            &validate_equipment_item(&dto, 5, 3),
            "name",
            "REQUIRED"
        ));
    }
    ...
    fn test_val_tr_itemized_empty_country_returns_error() {
        assert!(has_field_error(
            &validate_trip(&make_itemized_trip_dto(""), 3),
            "destination_country_code",
            "REQUIRED"
        ));
    }
    ...
    fn test_val_cfg_8_year_duration_returns_error() {
        assert!(has_field_error(
            &validate_project_config(&make_config_dto(8)),
            "duration_years",
            "INVALID_DURATION"
        ));
    }
    ...
    fn has_field_error(result: &Result<(), AppError>, field: &str, code: &str) -> bool {
        match result {
            Err(AppError::Validation(errs)) => errs
                .iter()
                .any(|e| e.field.as_deref() == Some(field) && e.code == code),
            _ => false,
        }
    }
    ...
    fn has_entity_error(result: &Result<(), AppError>, code: &str) -> bool {
        match result {
            Err(AppError::Validation(errs)) => {
                errs.iter().any(|e| e.field.is_none() && e.code == code)
            }
            _ => false,
        }
    }
