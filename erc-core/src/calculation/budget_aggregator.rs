...
pub fn calculate_total_eligible_costs(
    ...
    if total_direct < Decimal::ZERO {
        return Err(calc_error(
            "INTERNAL_CALC_ERROR",
            "Total direct costs is negative.",
        ));
    }
    if category_e < Decimal::ZERO {
        return Err(calc_error(
            "INTERNAL_CALC_ERROR",
            "Indirect costs is negative.",
        ));
    }
...
pub fn calculate_requested_contribution(total_eligible: Decimal) -> Result<Decimal, AppError> {
    ...
    if total_eligible < Decimal::ZERO {
        return Err(calc_error(
            "INTERNAL_CALC_ERROR",
            "Total eligible costs is negative.",
        ));
    }
...
mod tests {
    ...
    fn test_calc_13_duplicate_cfs_returns_error() {
        ...
        assert!(
            matches!(result, Err(AppError::Calculation { code, .. }) if code == "DUPLICATE_CFS_ITEM")
        );
    }
    ...
    fn test_calc_13_zero_amount_returns_error() {
        ...
        let result = aggregate_c3_costs(&items);
        assert!(
            matches!(result, Err(AppError::Calculation { code, .. }) if code == "INVALID_C3_AMOUNT")
        );
    }
    ...
    fn test_calc_14_indirect_25pct_excludes_b() {
        ...
        let result =
            calculate_indirect_costs(dec!(100), dec!(20), dec!(10), dec!(30), dec!(25)).unwrap();
    ...
    fn test_calc_14_zero_rate_gives_zero_indirect() {
        let result = calculate_indirect_costs(
            dec!(100000),
            Decimal::ZERO,
            Decimal::ZERO,
            Decimal::ZERO,
    ...
    fn test_calc_14_invalid_rate_over_50_returns_error() {
        ...
        assert!(
            matches!(result, Err(AppError::Calculation { code, .. }) if code == "INVALID_INDIRECT_RATE")
        );
    }
    ...
    fn test_calc_14_base_equals_sum_of_categories() {
        let result =
            calculate_indirect_costs(dec!(50000), dec!(10000), dec!(2000), dec!(3000), dec!(25))
                .unwrap();
    ...
    fn test_calc_16_negative_total_direct_returns_error() {
        ...
        assert!(
            matches!(result, Err(AppError::Calculation { code, .. }) if code == "INTERNAL_CALC_ERROR")
        );
    }
