...
mod tests {
    ...
    fn test_calc_05_zero_cost_returns_error() {
        ...
        assert!(
            matches!(result, Err(AppError::Calculation { code, .. }) if code == "INVALID_PURCHASE_COST")
        );
    }
    ...
    fn test_calc_05_zero_lifetime_returns_error() {
        ...
        assert!(
            matches!(result, Err(AppError::Calculation { code, .. }) if code == "INVALID_LIFETIME")
        );
    }
    ...
    fn test_calc_05_zero_usage_pct_returns_error() {
        ...
        assert!(
            matches!(result, Err(AppError::Calculation { code, .. }) if code == "INVALID_USAGE_PCT")
        );
    }
    ...
    fn test_calc_05_zero_usage_months_returns_error() {
        ...
        assert!(
            matches!(result, Err(AppError::Calculation { code, .. }) if code == "INVALID_USAGE_MONTHS")
        );
    }
