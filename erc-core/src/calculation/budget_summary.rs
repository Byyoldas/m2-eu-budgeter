...
use crate::calculation::{
    budget_aggregator::{
        aggregate_c3_costs, calculate_indirect_costs, calculate_requested_contribution,
        calculate_total_direct_costs, calculate_total_eligible_costs,
    },
    cfs_checker::check_cfs_threshold,
    equipment_depreciation::{aggregate_equipment_costs, calculate_depreciation},
    personnel_cost::{
        aggregate_personnel_costs, allocate_personnel_cost_by_wp, calculate_personnel_cost_lines,
        WpCostAmount,
    },
    salary_projection::{convert_try_to_eur, project_salary_chain},
    trip_cost::{
        aggregate_travel_costs, calculate_flat_trip_cost, calculate_itemized_trip_cost,
        TripCostResult,
    },
...
pub fn calculate_budget_summary(
        ...
        let trip_result: TripCostResult = match &trip.trip_type {
            TripType::Itemized {
                ...
                let detail = TripDetailDto {
                    ...
                    one_way_distance_km: Some(*one_way_distance_km),
                    number_of_nights: Some(*number_of_nights),
                    number_of_days: Some(*number_of_days),
                    flight_cost_per_instance: Some(r.flight_cost_per_instance.to_string()),
                    accommodation_cost_per_instance: Some(
                        r.accommodation_cost_per_instance.to_string(),
                    ),
                    subsistence_cost_per_instance: Some(
                        r.subsistence_cost_per_instance.to_string(),
                    ),
                    domestic_transport_per_instance: Some(
                        r.domestic_transport_per_instance.to_string(),
                    ),
    ...
    let c3_input: Vec<(Decimal, bool)> = project
        .other_cost_items
        .iter()
    ...
    let other_cost_wp_items: Vec<(Vec<u8>, Decimal)> = project
        .other_cost_items
        .iter()
        .map(|i| (i.work_package_ids.clone(), i.amount_eur))
        .collect();

    let other_cost_detail: Vec<OtherCostItemDetailDto> = project
        .other_cost_items
        .iter()
