...
mod tests {
    use super::*;
    use crate::domain::rate_data::{CountryRate, FlightBand, RateVersion};
    ...
    fn make_test_rate_version() -> RateVersion {
        RateVersion {
            ...
            flight_bands: vec![
                FlightBand {
                    band_id: "F-00".to_string(),
                    label: "Up to 399 km".to_string(),
                    min_km: 0,
                    max_km: 399,
                    cost_eur: 0,
                },
    ...
    fn test_calc_08_zero_nights_returns_error() {
        ...
        assert!(
            matches!(result, Err(AppError::Calculation { code, .. }) if code == "INVALID_NIGHTS")
        );
    }
    ...
    fn test_calc_08_unknown_country_returns_error() {
        ...
        assert!(
            matches!(result, Err(AppError::Calculation { code, .. }) if code == "COUNTRY_NOT_IN_RATE_TABLE")
        );
    }
    ...
    fn test_calc_10_negative_domestic_transport_returns_error() {
        ...
        assert!(
            matches!(result, Err(AppError::Calculation { code, .. }) if code == "INVALID_DOMESTIC_TRANSPORT")
        );
    }
    ...
    fn test_calc_10_zero_instances_returns_error() {
        ...
        assert!(
            matches!(result, Err(AppError::Calculation { code, .. }) if code == "INVALID_INSTANCES")
        );
    }
    ...
    fn test_calc_11_zero_flat_amount_returns_error() {
        ...
        assert!(
            matches!(result, Err(AppError::Calculation { code, .. }) if code == "INVALID_FLAT_AMOUNT")
        );
    }
