...
pub fn project_salary_chain(
    ...
    if !(1..=7).contains(&duration_years) {
        return Err(calc_error(
            "INVALID_DURATION",
...
mod tests {
    ...
    fn test_calc_01_zero_salary_returns_error() {
        ...
        assert!(
            matches!(result, Err(AppError::Calculation { code, .. }) if code == "INVALID_SALARY_TRY")
        );
    }
    ...
    fn test_calc_01_zero_rate_returns_error() {
        ...
        assert!(
            matches!(result, Err(AppError::Calculation { code, .. }) if code == "INVALID_EXCHANGE_RATE")
        );
    }
    ...
    fn test_calc_02_invalid_inflation_over_100() {
        ...
        assert!(
            matches!(result, Err(AppError::Calculation { code, .. }) if code == "INVALID_INFLATION_RATE")
        );
    }
    ...
    fn test_calc_02_zero_duration_returns_error() {
        ...
        assert!(
            matches!(result, Err(AppError::Calculation { code, .. }) if code == "INVALID_DURATION")
        );
    }
    ...
    fn test_calc_02_projections_ascending_order() {
        ...
        for i in 0..projections.len() {
            ...
            if i > 0 {
                assert!(
                    projections[i].projected_monthly_eur > projections[i - 1].projected_monthly_eur
                );
            }
        }
    }
